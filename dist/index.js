    ...
    for (const p of palettes) {
        ...
        for (const s of styles) {
            for (const c of contrasts) {
                ...
                themeWriter(`Ice ${p} ${c} ${s}`.replace(/ +/g, " ").trim(), Object.assign(Object.assign({}, commonColors), uiColors), semanticTheme.getPaletteRules(s), textmateTheme.getRules(s));
            }
        }
    }
}
...
function vimColorScheme() {
    fs.writeFile("./vim/colors/ice.vim", vimColoring(), (err) => {
        if (err) {
            throw err;
        }
